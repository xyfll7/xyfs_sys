...
async function base_fetch<T>(url: string, params: T) {
...
export const login = async (params: { code: string; }) => {
  return await base_fetch("/login", params);
};


export const auth_my_files = async (params: { cid: number; }) => {
  return await base_fetch("/auth/my-files", params);
};

export const auth_cate = async (params: { cid: number; }) => {
  return await base_fetch("/auth/cate", params);
};
