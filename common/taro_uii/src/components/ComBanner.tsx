...
export function ComBanner({
  ...
  const str___ = utils_str_includes(["bccwhite"], className) ? "linear-gradient(0deg, var(--color_white) 40%, rgba(0, 0, 0, 0) 100%);" : "linear-gradient(0deg, var(--color_back) 40%, rgba(0, 0, 0, 0) 100%);";
    ...
    <View className={`root-portal ${className}`} style={{ position: "absolute", top: "0vw", right: "0vw", width: "100vw", zIndex: -1 }}>
      <View className='ww'>
        ...
        <View className='pa ww transall5' style={{
          ...
          background: isHeaderBack ? "linear-gradient(0deg, var(--color_back) 95%, rgba(0, 0, 0, 0) 100%);" : str___,
        }} />
      </View>
    </View>
  </RootPortal>;
